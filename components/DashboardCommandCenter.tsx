        ...
        <section className={`${panel} order-3`} id="my-clubs" style={{ scrollMarginTop: "6rem" }}>
          ...
          {clubs.length ? <div className="grid gap-3 sm:grid-cols-2">{clubs.map((club) => {
            ...
            return <article key={club.id} className="min-w-0 overflow-hidden rounded-2xl border border-violet-300/15 bg-gradient-to-br from-violet-300/[0.06] to-slate-950/50 p-4">
              <Link href={`/clubs/${club.slug}`} className="block"><div className="flex items-center justify-between gap-2"><span aria-hidden="true" className="grid h-11 w-11 place-items-center overflow-hidden rounded-xl border border-white/10 bg-white/5 text-lg font-black text-violet-200">{club.logo_url ? <img src={club.logo_url} alt="" className="h-full w-full object-cover" /> : club.name.charAt(0).toUpperCase()}</span><span className="rounded-full bg-white/5 px-2.5 py-1 text-[0.6rem] font-black uppercase tracking-wider text-violet-200">{club.role}</span></div><h3 className="mt-4 break-words font-black text-white">{club.name}</h3><p className="mt-1 truncate text-xs text-slate-400">{club.location || "CueBracket club"}</p><p className="mt-3 text-xs text-slate-300">{club.memberCount ?? "—"} members · {data?.events ? clubEvents.length : "—"} upcoming events</p><span className="mt-4 inline-flex min-h-8 items-center text-xs font-black text-cyan-300">Open Command Center →</span></Link>
