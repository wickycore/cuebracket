...
async function getProfile(username: string) {
  ...
  const { data } = await supabase
    .from("profiles")
    .select("display_name, username, tournament_name, bio, avatar_url, created_at")
      ...
      <div className="mx-auto max-w-3xl px-5 py-10 sm:py-14">
        <section className="overflow-hidden rounded-[2rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.12),transparent_38%),linear-gradient(145deg,rgba(15,23,42,0.98),rgba(2,6,23,0.98))] shadow-2xl shadow-black/35">
          ...
          <div className="p-6 sm:p-9">
            ...
            <div className="mt-6 rounded-2xl border border-dashed border-white/10 px-5 py-4 text-sm leading-6 text-slate-500">
              Verified tournament history, podiums and club membership will appear here as the player ecosystem rolls out.
            </div>
          </div>
        </section>
      </div>
