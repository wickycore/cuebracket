...
export function PushNotificationSettings({ userId }: { userId: string }) {
  ...
  return <section id="phone-alerts" style={{ scrollMarginTop: "6rem" }} className="mt-6 rounded-3xl border border-cyan-300/20 bg-slate-900/65 p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="cb-kicker">Opt-in · This device</p><h2 className="mt-2 text-2xl font-black">Phone notifications</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Get club-event, registration and match/table updates even when CueBracket is closed. Player names and event details stay inside your signed-in inbox.</p></div><span className={`rounded-full px-3 py-2 text-xs font-black ${subscription ? "bg-emerald-300/10 text-emerald-200" : "bg-white/5 text-slate-400"}`}>{busy ? "Checking…" : subscription ? "On for this device" : "Off"}</span></div>
    ...
    <p className="mt-4 text-xs leading-5 text-slate-500">Signing out turns off phone alerts on this device. Each device needs its own permission; delivery depends on your connection and phone settings.</p>
  </section>;
}
