...
function SummaryCard({ value, label, detail, tone }: { value: number; label: string; detail: string; tone: string }) {
  return <article className="rounded-2xl border border-white/10 bg-slate-950/45 p-4 sm:p-5"><p className={`text-3xl font-black ${tone}`}>{value}</p><p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-white">{label}</p><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p></article>;
}
  ...
  return <div className="pb-16">
      ...
      <div className="cb-shell py-7 sm:py-10">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><p className="text-xs font-black uppercase tracking-[0.22em] text-amber-300">Private organizer workspace</p><span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[0.6rem] font-black uppercase tracking-wider text-slate-300">{props.role}</span></div>
    ...
    <nav aria-label="Club management sections" className="sticky top-16 z-30 border-b border-white/10 bg-[#06101f]/95 backdrop-blur-xl sm:top-[4.5rem]">
      <div className="cb-shell flex overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {sections.map((item) => <button key={item.id} type="button" onClick={() => chooseSection(item.id)} aria-current={activeSection === item.id ? "page" : undefined} className={`relative min-h-16 shrink-0 px-4 text-left sm:px-6 ${activeSection === item.id ? "text-white" : "text-slate-500 hover:text-slate-200"}`}><span className="block text-sm font-black">{item.label}</span><span className="mt-0.5 hidden text-[0.62rem] font-bold sm:block">{item.detail}</span>{activeSection === item.id ? <span className="absolute inset-x-4 bottom-0 h-0.5 rounded-full bg-gradient-to-r from-amber-300 to-cyan-300" /> : null}</button>)}
      </div>
    </nav>
