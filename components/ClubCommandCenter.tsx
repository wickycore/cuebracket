...
function MemberCard({ member, achievementCount }: { member: ClubMemberView; achievementCount: number }) {
  ...
  return <article className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/60 p-4">{member.username ? <Link href={`/players/${member.username}`} className="flex items-center gap-3">{content}</Link> : <div className="flex items-center gap-3">{content}</div>}{member.isPublic && member.username ? <FollowPlayerButton playerId={member.userId} profile={{ id: member.userId, username: member.username, display_name: member.name, tournament_name: member.name, avatar_url: member.avatarUrl, is_public: true }} /> : <p className="text-xs text-slate-400">A public player profile is needed to follow.</p>}</article>;
}
