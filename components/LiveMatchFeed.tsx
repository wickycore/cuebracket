...
export function LiveMatchFeed({ clubId, playerIds, enabled = true }: { clubId?: string; playerIds?: string[]; enabled?: boolean }) {
  ...
  return <section className="rounded-[2rem] border border-emerald-300/20 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,.09),transparent_24rem),rgba(15,23,42,.65)] p-5 sm:p-7">
    ...
    {!enabled || loading || (!error && loadedScope !== scope) ? <p className="mt-6 text-sm text-slate-400">Loading your watchboard…</p> : matches.length ? <div className="mt-5 grid gap-3 lg:grid-cols-2">{matches.map((match) => <Link key={liveMatchKey(match)} href={liveMatchHref(match)} className="block rounded-2xl border border-emerald-300/15 bg-slate-950/60 p-4 hover:border-emerald-300/40">
      <div className="flex items-start justify-between gap-3"><p className="min-w-0 truncate text-xs font-bold text-slate-400">{match.event_name}</p><span className="shrink-0 text-xs font-black uppercase tracking-wider text-emerald-300">● Live</span></div>
      ...
      <p className="mt-4 text-xs text-slate-400">{match.table_name || "Table not published"}<span className="float-right font-black text-emerald-300">Open match →</span></p>
    </Link>)}</div> : !error ? <div className="mt-5 rounded-2xl border border-dashed border-white/10 p-6"><p className="font-black text-slate-300">No linked players are live right now</p><p className="mt-2 text-sm leading-6 text-slate-400">{clubId ? "Matches appear when an organizer starts a public tournament match, or marks a linked league match as playing on a table." : "Follow public players to see their ongoing matches here. Turn on match alerts to hear when their next match starts."}</p></div> : null}
    {checkedAt && !error && enabled ? <p className="mt-4 text-xs text-slate-400">Last checked {checkedAt} · Up to 100 current matches · Scores depend on organizer cloud sync.</p> : null}
  </section>;
}
