...
export function ClubPracticeBoard({ clubId, clubSlug, userId, isMember, isAdmin, memberNames, initialChallenges }: Props) {
  ...
  return <section className="rounded-[2rem] border border-amber-300/15 bg-[radial-gradient(circle_at_top_right,rgba(251,191,36,.1),transparent_22rem),rgba(15,23,42,.65)] p-5 sm:p-7">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="cb-kicker !text-amber-300">Find your next match</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">Challenges & practice board</h2><p className="mt-2 text-sm leading-6 text-slate-400">Members can find a practice partner without filling the main club chat.</p></div><span className="rounded-full border border-amber-300/15 bg-amber-300/10 px-3 py-1.5 text-xs font-black text-amber-200">{visible.filter((item) => item.status === "open").length} open</span></div>
    ...
    {isMember ? <details className="mt-5 rounded-2xl border border-amber-300/15 bg-slate-950/40 p-4 sm:p-5"><summary className="cursor-pointer list-none font-black text-amber-100">+ Post a practice challenge</summary><form onSubmit={submit} className="mt-5 grid gap-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-black uppercase tracking-wider text-slate-400">Preferred time<input type="datetime-local" value={preferredAt} onChange={(event) => setPreferredAt(event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-slate-950 px-3 text-sm text-white" /></label><label className="text-xs font-black uppercase tracking-wider text-slate-400">Venue or table<input value={venue} onChange={(event) => setVenue(event.target.value)} maxLength={100} placeholder="Optional" className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-slate-950 px-4 text-sm font-bold text-white" /></label></div>
    ...
    </form></details> : userId ? <div className="mt-5 rounded-2xl border border-white/10 bg-slate-950/40 p-5"><p className="font-black text-white">Want to challenge a club player?</p><p className="mt-2 text-sm text-slate-400">Join the club first so the board stays trusted and useful.</p></div> : <Link href={`/auth/login?next=${encodeURIComponent(`/clubs/${clubSlug}?tab=clubhouse`)}`} className="mt-5 inline-flex rounded-xl bg-amber-300 px-5 py-3 font-black text-slate-950">Sign in to join the board</Link>}
    ...
    {visible.length ? <div className="mt-5 grid gap-4 lg:grid-cols-2">{visible.map((item) => {
      ...
      return <article key={item.id} className={`rounded-2xl border p-5 ${item.status === "matched" ? "border-emerald-300/20 bg-emerald-300/[0.045]" : "border-white/10 bg-slate-950/45"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-amber-300">{item.game_type === "any" ? "Any cue game" : item.game_type} · {item.skill_level === "any" ? "Any level" : item.skill_level}</p><h3 className="mt-2 text-lg font-black">{item.title}</h3><p className="mt-1 text-xs font-bold text-slate-400">Posted by {creator}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-black uppercase ${item.status === "matched" ? "bg-emerald-300/10 text-emerald-200" : "bg-amber-300/10 text-amber-200"}`}>{item.status}</span></div>
    ...
    })}</div> : <div className="mt-5 rounded-2xl border border-dashed border-white/10 p-8 text-center"><p className="font-black text-slate-300">No open practice challenges.</p><p className="mt-2 text-sm text-slate-400">Be the first member to call the next friendly match.</p></div>}
  </section>;
}
