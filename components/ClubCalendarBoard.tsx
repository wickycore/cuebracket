...
export function ClubCalendarBoard({ clubId, clubSlug, isAdmin, isMember, userId, initialEvents, initialRsvps }: Props) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    void run("create", async () => {
      ...
      setMessage("Club event added to the calendar.");
    });
  }
  ...
  return (
    <section className="rounded-[2rem] border border-emerald-300/15 bg-[radial-gradient(circle_at_top_right,rgba(52,211,153,.1),transparent_22rem),rgba(15,23,42,.65)] p-5 sm:p-7">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="cb-kicker !text-emerald-300">Plan together</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">Club calendar & RSVPs</h2><p className="mt-2 text-sm leading-6 text-slate-500">Practice nights, meetings and socials sit beside the tournament schedule.</p></div>
      ...
      {isAdmin ? <details className="mt-5 rounded-2xl border border-emerald-300/15 bg-slate-950/40 p-4 sm:p-5"><summary className="cursor-pointer list-none font-black text-emerald-100">+ Add a club calendar event</summary><form onSubmit={submit} className="mt-5 grid gap-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-black uppercase tracking-wider text-slate-500">Starts<input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} required className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-slate-950 px-3 text-sm text-white" /></label><label className="text-xs font-black uppercase tracking-wider text-slate-500">Ends (optional)<input type="datetime-local" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-slate-950 px-3 text-sm text-white" /></label></div>
      ...
      {events.length ? <div className="mt-5 grid gap-4 lg:grid-cols-2">{events.map((item) => {
        ...
        return <article key={item.id} className={`rounded-2xl border p-5 ${item.is_cancelled ? "border-rose-300/15 bg-rose-300/[0.035] opacity-75" : "border-white/10 bg-slate-950/45"}`}><div className="flex items-start justify-between gap-3"><div><span className={`rounded-full border px-2.5 py-1 text-[0.6rem] font-black uppercase tracking-wider ${kindTone[item.kind]}`}>{clubCalendarKindLabel(item.kind)}</span><h3 className="mt-3 text-lg font-black">{item.title}</h3></div>{item.is_cancelled ? <span className="text-xs font-black uppercase text-rose-300">Cancelled</span> : null}</div>
          <p className="mt-3 text-sm font-black text-white">{clubDateLabel(item.starts_at)}</p><p className="mt-1 text-sm font-bold text-slate-500">{item.location || "Location to be announced"}</p>{item.description ? <p className="mt-3 text-sm leading-6 text-slate-400">{item.description}</p> : null}
