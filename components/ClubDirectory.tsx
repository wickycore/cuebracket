...
export function ClubDirectory({ clubs }: { clubs: ClubWithCount[] }) {
  ...
  const visibleClubs = useMemo(() => {
    ...
    return clubs.filter((club) => {
      ...
      return matchesQuery && matchesLocation && (!verifiedOnly || club.is_verified);
    });
  }, [clubs, location, query, verifiedOnly]);

  return <>
    <div className="mt-5 grid gap-3 rounded-2xl border border-white/10 bg-slate-900/50 p-3 sm:grid-cols-[minmax(0,1fr)_15rem_auto]">
      ...
      <button type="button" aria-pressed={verifiedOnly} onClick={() => setVerifiedOnly((value) => !value)} className={`min-h-12 rounded-xl border px-4 text-sm font-black transition ${verifiedOnly ? "border-teal-300/40 bg-teal-300/15 text-teal-100" : "border-white/10 bg-slate-950 text-slate-300"}`}>✓ Verified only</button>
    </div>
    ...
    {visibleClubs.length ? <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {visibleClubs.map((club) => <Link key={club.id} href={`/clubs/${club.slug}`} className="group rounded-[1.75rem] border border-white/10 bg-slate-900/65 p-6 transition hover:-translate-y-0.5 hover:border-cyan-400/30 hover:bg-slate-900">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0"><h3 className="flex items-center gap-1.5 truncate text-xl font-black group-hover:text-cyan-200"><span className="truncate">{club.name}</span>{club.is_verified ? <ClubVerifiedBadge /> : null}</h3><p className="mt-1 truncate text-sm font-bold text-slate-400">{club.location || "Online club"}</p></div>
        </div>
    ...
    </div> : <div className="mt-5 rounded-[1.75rem] border border-dashed border-white/10 bg-white/[0.025] p-10 text-center"><p className="text-xl font-black">No clubs match those filters.</p><p className="mt-2 text-sm text-slate-400">Try another name or choose all locations.</p><button type="button" onClick={() => { setQuery(""); setLocation("all"); setVerifiedOnly(false); }} className="mt-4 rounded-xl border border-cyan-300/20 px-4 py-2 text-sm font-black text-cyan-200">Clear filters</button></div>}
  </>;
}
