        ...
        {visible.length ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {visible.map((item) => {
              ...
              const card = <>{item.image_url ? <div className="-mx-5 -mt-5 mb-5 h-40 overflow-hidden border-b border-white/10"><RemoteMedia src={item.image_url} alt={` achievement`} sizes="(max-width: 768px) 100vw, 33vw" /></div> : null}<div className="flex items-start justify-between gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl border border-amber-300/20 bg-amber-300/10 text-2xl">{clubAchievementIcon(item.kind)}</span>{item.is_featured ? <span className="rounded-full border border-amber-300/20 bg-amber-300/10 px-2.5 py-1 text-xs font-black uppercase tracking-wider text-amber-200">Featured</span> : null}</div><p className="mt-4 text-xs font-black uppercase tracking-[0.16em] text-amber-300">{clubAchievementLabel(item.kind)}</p><h3 className="mt-1 text-xl font-black text-white">{item.title}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-400">{item.description}</p><div className="mt-5 flex items-end justify-between gap-3 border-t border-white/8 pt-4"><span className="min-w-0"><span className="block truncate text-sm font-black text-white">{member?.name ?? "Former club member"}</span><span className="mt-0.5 block text-xs font-bold text-slate-400">{achievementDate(item.awarded_on)}</span></span>{member?.username ? <span className="shrink-0 text-xs font-black text-cyan-300">View player →</span> : null}</div></>;
