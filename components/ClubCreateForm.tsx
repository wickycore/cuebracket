...
export function ClubCreateForm() {
  ...
  const inputClass = "mt-2 min-h-12 w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white outline-none placeholder:text-slate-400 focus:border-cyan-400/55 focus:ring-4 focus:ring-cyan-400/10";
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-bold text-slate-300">
          ...
          <div className="relative">
            <span className="pointer-events-none absolute left-4 top-1/2 mt-1 -translate-y-1/2 text-xs font-bold text-slate-400">/clubs/</span>
        ...
        <label className="text-sm font-bold text-slate-300 sm:col-span-2">
          ...
          <span className="mt-2 block text-right text-xs text-slate-400">{description.length}/500</span>
        </label>
        <label className="text-sm font-bold text-slate-300 sm:col-span-2">
          Club logo or image <span className="font-normal text-slate-400">(optional)</span>
          <span className="mt-2 flex items-center gap-4 rounded-2xl border border-white/10 bg-slate-950/70 p-4">
            ...
            <span className="min-w-0 flex-1"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0] ?? null; const problem = file ? validateImageFile(file) : null; if (problem) { setMessage(problem); event.target.value = ""; return; } setLogoFile(file); setLogoPreview(file ? URL.createObjectURL(file) : ""); setMessage(""); }} className="block w-full text-xs text-slate-400 file:mr-3 file:rounded-xl file:border-0 file:bg-cyan-400 file:px-4 file:py-2.5 file:font-black file:text-slate-950" /><span className="mt-2 block text-xs font-normal text-slate-400">JPG, PNG or WebP · maximum 5 MB</span></span>
          </span>
        </label>
      </div>
