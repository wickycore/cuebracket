...
function EventCard({ event }: { event: DashboardEvent }) {
  return <Link href={event.href} className="group flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/35 p-4 transition hover:border-cyan-300/30 sm:gap-4">
    ...
    <span className="min-w-0 flex-1"><span className="block truncate font-black text-white group-hover:text-cyan-100">{event.name}</span><span className="mt-1 block truncate text-xs text-slate-400">{event.detail}</span><span className="mt-1 block text-xs text-slate-400">{event.total ? `${event.completed}/${event.total} results` : "Setup in progress"} · {dashboardDate(event.updatedAt)}</span></span>
    <span className={`shrink-0 rounded-full border px-2.5 py-1.5 text-xs font-black uppercase tracking-wider ${statuses[event.status]}`}>{event.status}</span>
  </Link>;
}
  ...
  return <div className="cb-shell space-y-6 py-6 pb-14 sm:space-y-8 sm:py-8">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.55fr)_minmax(18rem,1fr)]">
      <section className="relative min-w-0 overflow-hidden rounded-3xl border border-cyan-300/20 bg-[radial-gradient(ellipse_at_top_right,rgba(34,211,238,.12),transparent_70%),linear-gradient(125deg,#102a43,#071425)] p-5 sm:p-7">
        ...
        <div className="flex items-center justify-between gap-3"><p className="cb-kicker">Continue where you stopped</p>{resume ? <span className={`rounded-full border px-3 py-1 text-xs font-black uppercase ${statuses[resume.status]}`}>{resume.status === "live" ? "● Live event" : "Draft"}</span> : <span className="text-xs font-bold text-cyan-200">Your next move</span>}</div>
    ...
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(19rem,1fr)]">
        ...
        <section className={`${panel} order-3`} id="my-clubs" style={{ scrollMarginTop: "6rem" }}>
          ...
          {clubs.length ? <div className="grid gap-3 sm:grid-cols-2">{clubs.map((club) => {
            ...
            return <article key={club.id} className="min-w-0 overflow-hidden rounded-2xl border border-violet-300/15 bg-gradient-to-br from-violet-300/[0.06] to-slate-950/50 p-4">
              <Link href={`/clubs/${club.slug}`} className="block"><div className="flex items-center justify-between gap-2"><span aria-hidden="true" className="grid h-11 w-11 place-items-center overflow-hidden rounded-xl border border-white/10 bg-white/5 text-lg font-black text-violet-200">{club.logo_url ? <RemoteMedia src={club.logo_url} alt={` logo`} width={88} height={88} sizes="44px" /> : club.name.charAt(0).toUpperCase()}</span><span className="rounded-full bg-white/5 px-2.5 py-1 text-xs font-black uppercase tracking-wider text-violet-200">{club.role}</span></div><h3 className="mt-4 break-words font-black text-white">{club.name}</h3><p className="mt-1 truncate text-xs text-slate-400">{club.location || "CueBracket club"}</p><p className="mt-3 text-xs text-slate-300">{club.memberCount ?? "—"} members · {data?.events ? clubEvents.length : "—"} upcoming events</p><span className="mt-4 inline-flex min-h-8 items-center text-xs font-black text-cyan-300">Open Command Center →</span></Link>
      ...
      <div className="contents">
        <section className={`${panel} order-2`}>
          ...
          {upcoming.length ? <div className="space-y-3">{upcoming.slice(0, 5).map((event) => <Link key={event.tournament_id} href={`/register/${event.tournament_id}`} className="block rounded-2xl border border-white/10 bg-slate-950/35 p-4 hover:border-cyan-300/30"><p className="text-xs font-black text-cyan-200">{dashboardDate(event.scheduled_at!, true)} EAT</p><h3 className="mt-2 font-black text-white">{event.event_name}</h3><p className="mt-1 text-xs text-slate-400">{event.venue || "Venue to be announced"}</p><div className="mt-3 flex items-center justify-between gap-2 text-xs font-bold"><span className="truncate text-slate-400">{clubs.find((club) => club.id === event.club_id)?.name || "Your event"}</span><span className={event.registration_open ? "text-emerald-300" : "text-slate-400"}>{event.registration_open ? "Entries open →" : "View event →"}</span></div></Link>)}</div> : <Empty title={!data ? loading ? "Loading your calendar…" : "Calendar unavailable" : data.events === null ? "Calendar unavailable" : "Your calendar is clear"} text="Upcoming events you organize, enter or follow through a club will show here." href="/events" action="Find an event" />}
        </section>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-center">
      <div className="min-w-0">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1">
          ...
          {progress.publicHref ? <Link href={progress.publicHref} className="inline-flex min-h-11 items-center text-sm font-bold text-slate-300">View public profile →</Link> : <span className="text-xs text-slate-400">{data.profile?.is_public === false ? "Profile visibility: private" : "You control your profile visibility"}</span>}
        </div>
      </div>
      <div>
...
function Attention({ href, value, title, detail }: { href: string; value: number | null | undefined; title: string; detail: string }) {
  return <Link href={href} className="flex items-center gap-3 rounded-xl border border-white/8 bg-slate-950/35 p-3.5 transition hover:border-cyan-300/25"><span className={`grid h-10 min-w-10 shrink-0 place-items-center rounded-xl px-2 text-lg font-black tabular-nums ${value ? "bg-amber-300/10 text-amber-200" : "bg-white/5 text-slate-400"}`}>{value ?? "—"}</span><span className="min-w-0"><span className="block text-sm font-bold text-slate-100">{title}</span><span className="mt-1 block text-xs leading-5 text-slate-400">{detail}</span></span><span aria-hidden="true" className="ml-auto text-slate-400">→</span></Link>;
}
