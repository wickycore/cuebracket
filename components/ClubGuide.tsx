...
export function ClubGuide({ clubId, isAdmin, location }: { clubId: string; isAdmin: boolean; location: string }) {
  ...
  return <section className="rounded-[2rem] border border-white/10 bg-slate-900/60 p-5 sm:p-7">
    ...
    {loading ? <p className="mt-4 text-slate-400">Loading guide…</p> : loadError ? <button type="button" onClick={() => { setLoading(true); setRetry((n) => n + 1); }} className="mt-4 text-sm text-cyan-300 underline">Retry loading guide</button> : editing ? <form className="mt-5 space-y-4" onSubmit={(event) => { event.preventDefault(); void save(); }}>
      <label className="block text-sm font-bold text-slate-300">Pool club location<input value={venueLocation} onChange={(event) => setVenueLocation(event.target.value)} maxLength={100} required placeholder="e.g. Mwiki, Kasarani" className="mt-2 block min-h-12 w-full rounded-xl border border-white/15 bg-slate-950 px-3 text-sm font-normal" /><span className="mt-2 block text-xs font-normal text-slate-400">Members will see this location and can open it in Maps before accepting the guide.</span></label>
    ...
    </form> : <div className="mt-5 grid gap-5 sm:grid-cols-2"><div><h3 className="text-xs font-black uppercase tracking-wider text-slate-400">Location & opening hours</h3><p className="mt-2 font-bold text-white">{venueLocation ? `📍 ${venueLocation}` : "Location has not been published."}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-300">{hours || "Hours have not been published yet. Check with the club before visiting."}</p>{venueLocation ? <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(venueLocation)}`} target="_blank" rel="noopener noreferrer" className="mt-4 inline-block text-sm font-black text-cyan-300">Open location in Maps ↗</a> : null}</div><div><h3 className="text-xs font-black uppercase tracking-wider text-slate-400">House rules & joining guide</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-300">{rules || "The club has not published its joining guide yet."}</p>{revision ? <p className="mt-3 text-xs font-black uppercase tracking-wider text-slate-400">Guide version {revision}</p> : null}</div></div>}
